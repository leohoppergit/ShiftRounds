package de.nulide.shiftcal.net

import android.content.Context
import com.android.volley.RequestQueue
import com.android.volley.toolbox.HurlStack
import com.android.volley.toolbox.Volley
import de.nulide.shiftcal.crypt.Base64Helper
import java.io.ByteArrayInputStream
import java.io.IOException
import java.security.KeyManagementException
import java.security.KeyStore
import java.security.KeyStoreException
import java.security.NoSuchAlgorithmException
import java.security.cert.CertificateException
import java.security.cert.CertificateFactory
import javax.net.ssl.SSLContext
import javax.net.ssl.TrustManagerFactory

object PatchedVolley {
    private const val ISRGROOTX1 =
        ""

    fun newRequestQueue(context: Context): RequestQueue {
        try {
            val baseTrustStore = KeyStore.getInstance(KeyStore.getDefaultType())
            baseTrustStore.load(null, null)
            val cf = CertificateFactory.getInstance("X.509")
            val myTrustedCertificate = cf.generateCertificate(
                ByteArrayInputStream(
                    Base64Helper.from(
                        ISRGROOTX1
                    )
                )
            )
            baseTrustStore.setCertificateEntry("my-trusted-certificate", myTrustedCertificate)

            var counter = 0

            val systemTrustStore = KeyStore.getInstance("AndroidCAStore")
            systemTrustStore.load(null, null)
            val aliases = systemTrustStore.aliases()
            while (aliases.hasMoreElements()) {
                val alias = aliases.nextElement()
                if (systemTrustStore.isCertificateEntry(alias)) {
                    val certificate = systemTrustStore.getCertificate(alias)
                    baseTrustStore.setCertificateEntry("" + counter++, certificate)
                }
            }

            val trustManagerFactory =
                TrustManagerFactory.getInstance(TrustManagerFactory.getDefaultAlgorithm())
            trustManagerFactory.init(baseTrustStore)
            val trustManagers = trustManagerFactory.trustManagers

            val sslContext = SSLContext.getInstance("TLS")
            sslContext.init(null, trustManagers, null)
            val socketFactory = sslContext.socketFactory

            val hurlStack = HurlStack(null, socketFactory)
            return Volley.newRequestQueue(context, hurlStack)
        } catch (e: CertificateException) {
            e.printStackTrace()
        } catch (e: IOException) {
            e.printStackTrace()
        } catch (e: NoSuchAlgorithmException) {
            e.printStackTrace()
        } catch (e: KeyStoreException) {
            e.printStackTrace()
        } catch (e: KeyManagementException) {
            e.printStackTrace()
        }
        return Volley.newRequestQueue(context)
    }
}